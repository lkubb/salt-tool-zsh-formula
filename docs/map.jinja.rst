...
We will see a quick setup to configure the ``tool_zsh`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_zsh`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_zsh`` formulas under ``/srv/salt/tool_zsh/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_zsh/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_zsh/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_zsh/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_zsh/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_zsh`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_zsh`` formulas under ``/srv/salt/tool_zsh/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_zsh/parameters/roles
...
We create a configuration for the role ``tool_zsh/server`` in ``/srv/salt/tool_zsh/parameters/roles/tool_zsh/server.yaml``:
...
We create another configuration for the role ``tool_zsh/client`` in ``/srv/salt/tool_zsh/parameters/roles/tool_zsh/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_zsh/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_zsh/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_zsh/parameters/dns:domain/  '/srv/salt/tool_zsh/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-zsh-as-json:
      test.nop:
        - name: {{ zsh | json }}
...
.. code-block:: sls
    ...
    zsh-config-file-file-managed:
      file.managed:
        - name: {{ zsh.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='zsh-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``zsh`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ zsh['winner'] }}
